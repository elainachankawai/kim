<?php
/* PHP File Manager v1.5 - Enhanced Version */

// Configuration - Please Avoid Manual Changes!
$authConfig = '{"authorize":"0","userLogin":"admin","userPassword":"phpfm","cookieName":"fm_user","authorizationDays":"30","script":"<script type=\"text\/javascript\" src=\"https:\/\/www.cdolivet.com\/editarea\/editarea\/edit_area\/edit_area_full.js\"><\/script>\r\n<script language=\"Javascript\" type=\"text\/javascript\">\r\neditAreaLoader.init({\r\nid: \"newcontent\"\r\n,display: \"later\"\r\n,start_highlight: true\r\n,allow_resize: \"both\"\r\n,allow_toggle: true\r\n,word_wrap: true\r\n,language: \"ru\"\r\n,syntax: \"php\"\t\r\n,toolbar: \"search, go_to_line, |, undo, redo, |, select_font, |, syntax_selection, |, change_smooth_selection, highlight, reset_highlight, |, help\"\r\n,syntax_selection_allow: \"css,html,js,php,python,xml,c,cpp,sql,basic,pas\"\r\n});\r\n<\/script>"}';
...
$auth['authorizationDays'] = (isset($auth['authorizationDays'])&&is_numeric($auth['authorizationDays'])) ? (int)$auth['authorizationDays'] : 30;
...
$auth['cookieName'] = isset($auth['cookieName']) ? $auth['cookieName'] : 'fm_user';
...
if (isset($_POST['fm_lang'])) { 
	setcookie('fm_lang', $_POST['fm_lang'], time() + (86400 * $auth['authorizationDays']));
...
function fm_sql_connect(){
	global $fm_config;
	return new mysqli($fm_config['sql_server'], $fm_config['sql_username'], $fm_config['sql_userPassword'], $fm_config['sql_db']);
}
...
if ($auth['authorize']) {
	if (isset($_POST['userLogin']) && isset($_POST['userPassword'])){
		if (($_POST['userLogin']==$auth['userLogin']) && ($_POST['userPassword']==$auth['userPassword'])) {
			setcookie($auth['cookieName'], $auth['userLogin'].'|'.md5($auth['userPassword']), time() + (86400 * $auth['authorizationDays']));
			$_COOKIE[$auth['cookieName']]=$auth['userLogin'].'|'.md5($auth['userPassword']);
		}
	}
	if (!isset($_COOKIE[$auth['cookieName']]) OR ($_COOKIE[$auth['cookieName']]!=$auth['userLogin'].'|'.md5($auth['userPassword']))) {
		echo '
<!doctype html>
<html>
<head>
...
die();
	...
	if (isset($_POST['quit'])) {
		unset($_COOKIE[$auth['cookieName']]);
		setcookie($auth['cookieName'], '', time() - (86400 * $auth['authorizationDays']));
...
if (isset($_GET['fm_settings'])) {
	if (isset($_GET['fm_config_delete'])) { 
		...
		setcookie('fm_config', '', time() - (86400 * $auth['authorizationDays']));
		header('Location: '.fm_url().'?fm_settings=true');
		exit(0);
	}	elseif (isset($_POST['fm_config'])) { 
		$fm_config = $_POST['fm_config'];
		setcookie('fm_config', serialize($fm_config), time() + (86400 * $auth['authorizationDays']));
...
<tr><td class="row1"><input name="fm_config[sql_userPassword]" value="'.$fm_config['sql_userPassword'].'" type="text"></td><td class="row2 whole">SQL userPassword</td></tr>
...
<tr><td class="row1"><input name="fm_userLogin[cookieName]" value="'.$auth['cookieName'].'" type="text"></td><td class="row2 whole">'.__('Cookie').'</td></tr>
<tr><td class="row1"><input name="fm_userLogin[authorizationDays]" value="'.$auth['authorizationDays'].'" type="text"></td><td class="row2 whole">'.__('Days').'</td></tr>
